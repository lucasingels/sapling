...
5. **`sl` built-in config** (`eden/scm/lib/config/loader/src/builtin_static/core.rs`, or the `sl_oss` static config): `edenfs.command` and `edenfs.legacy_command` default to the sibling `eden` when `sl` was built with the `eden` feature. Without these `sl worktree add` aborts with "edenfs.legacy_command config is not set". (The `worktree.enabled` gate no longer exists: upstream removed it in `05a48716697`, taken in the 2026-09-04 merge, so nothing needs to default it.)
