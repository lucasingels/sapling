...
5. **Setuid privhelper via caveats, sudo fallback as safety net.** Homebrew cannot install setuid-root files. The formula's `caveats` print the one-time `sudo chown root:wheel … && sudo chmod 4755 …` for `libexec/eden/edenfs_privhelper` (needed again after every upgrade). If the user skips it, `eden start` already wraps the daemon in `sudo` and prompts, which is upstream's own dev-machine behaviour.
