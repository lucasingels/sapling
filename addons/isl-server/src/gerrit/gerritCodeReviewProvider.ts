...
export class GerritCodeReviewProvider implements CodeReviewProvider {
  ...
  /**
   * Query the Gerrit REST API for the given Change-Ids and emit updated summaries.
   * Debounced with leading-edge firing, matching the GitHub provider's pattern.
   */
  triggerDiffSummariesFetch = debounce(
    async (diffs: Array<DiffId>) => {
      if (diffs.length === 0) {
      ...
      try {
        ...
        // Build an OR query: change:Id1+OR+change:Id2+...
        const query = diffs.map(id => `change:${encodeURIComponent(id)}`).join('+OR+');
        ...
        this.logger.info(`Gerrit: fetching ${diffs.length} change(s) from REST API`);
